...
type BranchComparisonCreateOpts struct {
	XPhraseAppOTP optional.String `json:"X-PhraseApp-OTP,omitempty"`
}
...
Create a branch comparison asynchronously.
  ...
  - @param projectId Project ID
  - @param name name
  - @param branchCreateComparisonParameters
  - @param optional nil or *BranchComparisonCreateOpts - Optional Parameters:
  - @param "XPhraseAppOTP" (optional.String) -  Two-Factor-Authentication token (optional)
*/
func (a *BranchesApiService) BranchComparisonCreate(ctx _context.Context, projectId string, name string, branchCreateComparisonParameters BranchCreateComparisonParameters, localVarOptionals *BranchComparisonCreateOpts) ([]byte, *APIResponse, error) {
	var (
	...
	// body params
	localVarPostBody = &branchCreateComparisonParameters
	if ctx != nil {
		// API Key Authentication
		if auth, ok := ctx.Value(ContextAPIKey).(APIKey); ok {
			var key string
...
// BranchSyncOpts Optional parameters for the method 'BranchSync'
...
Sync an existing branch.  *Note: Only available for branches created with new branching.*
  ...
  - @param projectId Project ID
  - @param name name
  - @param branchSyncParameters
  - @param optional nil or *BranchSyncOpts - Optional Parameters:
  - @param "XPhraseAppOTP" (optional.String) -  Two-Factor-Authentication token (optional)
*/
func (a *BranchesApiService) BranchSync(ctx _context.Context, projectId string, name string, branchSyncParameters BranchSyncParameters, localVarOptionals *BranchSyncOpts) ([]byte, *APIResponse, error) {
	var (
		localVarHTTPMethod   = _nethttp.MethodPatch
		localVarPostBody     interface{}
		localVarFormFileName string
	...
	// create path and map variables
	localVarPath := a.client.cfg.BasePath + "/projects/{project_id}/branches/{name}/sync"
	localVarPath = strings.Replace(localVarPath, "{"+"project_id"+"}", _neturl.QueryEscape(parameterToString(projectId, "")), -1)
	...
	// body params
	localVarPostBody = &branchSyncParameters
	if ctx != nil {
		// API Key Authentication
		if auth, ok := ctx.Value(ContextAPIKey).(APIKey); ok {
			var key string
